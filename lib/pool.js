...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    function OnBlockchainSynced(syncedCallback){
        ...
        var generateProgress = function(){
            ...
            _this.daemon.cmd('getblockchaininfo', [], function(results) {	// new api
//            _this.daemon.cmd('getinfo', [], function (results) {			// old api
        ...
        _this.jobManager.on('newBlock', function(blockTemplate){
            ...
            */
            if (!isValidBlock)
				...
				console.log("check sharedata.blockHash:" + shareData.blockHash);
...
//            ['getinfo', []],		// old api
        ...
        _this.daemon.batchCmd(batchRpcCalls, function(error, results){
            ...
            options.testnet = (rpcResults.getblockchaininfo.chain === 'test') ? true : false;	// new api
            // options.testnet = rpcResults.getinfo.testnet;	// old api

            options.protocolVersion = rpcResults.getnetworkinfo.protocolversion;		// new api
            // options.protocolVersion = rpcResults.getinfo.protocolversion;		// old api


            options.initStats = {
                connections: rpcResults.getnetworkinfo.connections,					// new api
                difficulty: rpcResults.getblockchaininfo.difficulty * algos[options.coin.algorithm].multiplier,		// new api
                // connections: rpcResults.getnetworkinfo.connections,					// old api
                // difficulty: rpcResults.getblockchaininfo.difficulty * algos[options.coin.algorithm].multiplier,		// old api
