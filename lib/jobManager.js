...
var JobManager = module.exports = function JobManager(options){
    ...
    var blockHasher = (function () {
        switch (options.coin.algorithm) {
            ...
            default:
                return function () {
//		    console.log("DEF::Blockhash::hashdigest(" + mydump(this,1) + " , args [" + mydump(arguments,1) + " ]");
//		    console.log("DEF::Blockhash::hashdigest( this"); console.log(" , args ["); console.log(arguments); console.log(" ]\n");
		    console.log("DEF::Blockhash::hashdigest( this[" + options.coin.algorithm + "]");
                    return util.reverseBuffer(hashDigest.apply(this, arguments));
                };
        }
    })();
	...
	var blockHashscrypt;
        ...
        if (job.target.ge(headerBigNum)){
            ...
            if (options.coin.sha256DHeader) {
                ...
                console.log("sha256d::blockHash = %", blockHash);
            }
            else if (options.coin.scryptHeader) {
		// VERGE myriadgroestl uses scrypt headerhashing

		blockHashScrypt = util.reverseBuffer(multiHashing.scrypt(headerBuffer, 1024,1));
	        blockHashorg = blockHash;
		blockHash = blockHashScrypt.toString('hex');
            }
            ...
            if (options.emitInvalidBlockHashes) {
            	blockHashInvalidorg = blockHasher(headerBuffer, nTime).toString('hex');
		console.log("Before invalid options.coin.forceSha256Dblockhash (BlockHash): " + blockHashInvalidorg);

            	if (options.coin.sha256DHeader) {
               	    blockHashInvalid = util.reverseBuffer(util.sha256d(headerBuffer)).toString('hex');
                    console.log("sha256d::blockHashInvalid = %", blockHashInvalid);
                }
                else if (options.coin.scryptHeader) {
	            // VERGE myriadgroestl uses scrypt headerhashing

		    blockHashScrypt = util.reverseBuffer(multiHashing.scrypt(headerBuffer, 1024,1));
	            blockHashorg = blockHashInvalid;
		    blockHashInvalid = blockHashScrypt.toString('hex');
		}
		console.log("after invalid options.coin.forceSha256Dblockhash: " + blockHashInvalid);
        ...
        _this.emit('share', {
            ...
            blockHashInvalidorg: blockHashInvalidorg
        }, blockHex);
